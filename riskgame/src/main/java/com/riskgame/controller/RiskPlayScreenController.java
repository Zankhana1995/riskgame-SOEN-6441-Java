...
public class RiskPlayScreenController implements Initializable {
	...
	@FXML
	private TextArea continentArea;
	...
	private static int attackerTotalDice = 3;
	private static int defenderTotalDice = 2;

	private static String countryNameFrom = new String();
	private static String countryNameTo = new String();
	 ...
	 * player and performs action accordingly. Fortification
	 * 
	...
	void fireCommand(ActionEvent event) {
		...
		try {
			...
			if (command != null && !command.isEmpty()) {
				if (command.startsWith("reinforce")) {
					...
					if (mapConquered) {
						 ...
						 * txtConsoleLog.setText(attackPhase(command)); } else if
						 * (command.startsWith("defend")) {
						 * 
						 * } else {
						 * 
						 * }
			...
			/*
			 * else { txtConsoleLog.setText("Please Enter valid Command"); }
			 */
		} catch (Exception e) {
			e.printStackTrace();
			txtConsoleLog.setText("Please Enter Valid Command");
	...
	/**
	 * This a attack phase method in which players whose turn is active will be
	 * choosing source country and destination country and declare an attack.
	 * players and defenders get to choose number of dices and outcome of attack
	 * phase will be determined.
	 * 
	 * @param command is a command fired from player
	 * @return message of result from attackphase process
	 */
	private String attackPhase(String command) {
		...
		if (commandData.get(0).equals("attack") && commandData.get(3).equals("-allout")) {
			if (commandData.size() == 4 && validateInput(commandData.get(1), "^([a-zA-Z]-+\\s)*[a-zA-Z-]+$")
				...
				System.out.println(attackerTotalDice);
				System.out.println(defenderTotalDice);
				System.out.println("allout");
			}
		} else if (commandData.get(0).equals("attack") && commandData.get(1).equals("-noattack")) {

		} else if (commandData.get(0).equals("attack")) {
			countryNameFrom = commandData.get(1);
			countryNameTo = commandData.get(2);
			attackerTotalDice = Integer.parseInt(commandData.get(3));
			if (commandData.size() == 4 && validateInput(commandData.get(1), "^([a-zA-Z]-+\\s)*[a-zA-Z-]+$")
					&& validateInput(commandData.get(2), "^([a-zA-Z]-+\\s)*[a-zA-Z-]+$")
					&& validateInput(commandData.get(3), "[1-9][0-9]*") && attackerTotalDice < 4) {

				System.out.println("attackdice" + attackerTotalDice);
			}
		} else if (commandData.get(0).equals("defend")) {
			...
			for (int i = 0; i < otherPlayers.size(); i++) {
				System.out.println("ATTACK ====> " + playerList.get(otherPlayers.get(i)));
				Player defenderPlayers = playerList.get(i);
				fillContinentArea(defenderPlayers);
			}

		} else if (commandData.get(0).equals("attackmove")) {
	...
	private ArrayList<Integer> rollDice(int attackerDice, int defenderDice) {
	...
	private void fillContinentArea(Player defenderPlayers) {
		//continentArea.clear();
		StringBuilder sbBuilder = new StringBuilder();
		if (defenderPlayers != null) {
			List<PlayerTerritory> playerTerritories = defenderPlayers.getPlayerterritories();
			int count = 1;
			for (PlayerTerritory territory : playerList.get(defenderPlayers.getPlayerId()).getPlayerterritories()) {
				...
				for (String neighbour : neighbourCountriesList) {
					...
					PlayerTerritory playerTerritory = playerList.get(defenderPlayers.getPlayerId())
							.getPlayerterritories().stream().filter(x -> neighbour.equals(x.getTerritoryName()))
							.findAny().orElse(null);

					if (playerTerritory != null) {
						// System.out.println("ATTACK ====> "+count+"
						// "+playerTerritory.getTerritoryName());
						sbBuilder.append(" ==> ").append(playerTerritory.getTerritoryName());
					}
			...
			continentArea.setText(sbBuilder.toString());
		}
	}
